...
export function renderPortfolio(ctx) {
  ...
  const body=`${breadcrumbs([{label:'Inicio',url:'/'},{label:'Proyectos'}])}<section class="inner-hero"><div class="container narrow reveal"><span class="eyebrow">Proyectos seleccionados</span><h1>Algunos proyectos en los que he trabajado</h1><p>No quiero llenar esta página con todo lo que he hecho. Prefiero mostrar una selección de trabajos que representan distintos retos de diseño, desarrollo, comunicación y conversión.</p></div></section><section class="section"><div class="container project-grid portfolio-grid">${portfolio.map((project,index)=>`<article class="project-card reveal" id="${['dabar','stageg','makrisystem','futura','vida-activa'][index]}"><img src="${project.image}" width="760" height="500" loading="${index<2?'eager':'lazy'}" decoding="async" alt="${esc(project.imageAlt)}"><div class="project-body"><span>${esc(project.category)}</span><h2>${esc(project.name)}</h2><p>${esc(project.summary)}</p><small>${esc(project.result)}</small>${project.url.startsWith('/casos')?`<a class="text-link" href="${project.url}">Ver caso de éxito ${icons.arrow}</a>`:''}</div></article>`).join('')}</div></section>${cta(site,'¿Tienes un proyecto que quieres poner en marcha?','Cuéntame qué necesitas construir o mejorar. Si puedo ayudarte, te explicaré cómo lo plantearía y qué alcance tendría.')}`;
